...
Auth (pick one):
  - GOOGLE_CLIENT_SECRET_JSON: full client secret JSON (for GitHub Actions)
  - GOOGLE_CLIENT_SECRET_FILE: path to client_secret*.json (local default)
  - YOUTUBE_REFRESH_TOKEN: long-lived refresh token (required for uploads)
...
def build_title_description(caption, used_quote=None):
    """YouTube title max 100 chars; description can include full caption + hashtags."""
    if used_quote:
...
def upload_short(
    ...
    *,
    ...
    """
    Upload a local MP4 as a YouTube video (works for Shorts: vertical, short duration).
    ...
    _debug(f"title={title!r} privacy={privacy!r} tags_count={len(tags)}")
